...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get the form data
    $course_name = $_POST['course_name'];  // Course name (e.g., 'Math')
    ...
    $section_table = "batch_39_1st_semester_" . ucfirst(strtolower($course_name)) . "_section_" . strtolower($section);

    // Step 1: Check if the section has space (i.e., less than 5 students)
    $result = $conn->query("SELECT current_students FROM $section_table WHERE course_name = '$course_name'");
    ...
    if ($current_students_in_section >= $max_students) {
        ...
        $conn->query("INSERT INTO $section_table (student_id, course_name) VALUES ('$student_id', '$course_name')");

        // Step 4: Update the `current_students` count in the section table
        $conn->query("UPDATE $section_table SET current_students = current_students + 1 WHERE course_name = '$course_name'");

        // Step 5: Update the `current_students` in the main course table
        $conn->query("UPDATE batch_39_1st_semester_courses SET current_students = current_students + 1 WHERE id = '$course_id'");
